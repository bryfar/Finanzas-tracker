  ...
  return (
    <div className="min-h-[100dvh] bg-surface font-sans text-slate-800 flex overflow-hidden">
        ...
        <aside className="hidden lg:flex flex-col w-72 p-4 h-screen sticky top-0 z-40">
            <div className="bg-white rounded-[2rem] shadow-soft border border-slate-100/50 h-full flex flex-col p-6 relative overflow-hidden backdrop-blur-xl transition-all hover:shadow-xl">
                 <div className="flex items-center gap-3 mb-8 relative z-10 group cursor-default">
                     <div className="w-12 h-12 relative transition-transform group-hover:scale-110 duration-300">
                        ...
                        <Mascot variant="idle" size={48} className="relative z-10" />
                     </div>
                     <div>
                        ...
                        <span className="text-[9px] text-brand-500 font-bold uppercase tracking-[0.2em] bg-brand-50 px-2 py-0.5 rounded-md inline-block mt-1">Premium</span>
                     </div>
                 </div>
                 ...
                 <nav className="space-y-1 flex-1 relative z-10">
                    {[
                        ...
                        <button 
                            ...
                            className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-2xl font-heading font-bold text-xs transition-all duration-300 group relative overflow-hidden ${activeView === item.id ? 'bg-brand-600 text-white shadow-lg shadow-brand-500/30' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
                        >
                            <item.icon size={18} className={`transition-transform duration-300 relative z-10 ${activeView === item.id ? 'scale-110' : 'group-hover:scale-110'}`} /> 
                 ...
                 <div className="mt-auto relative z-10">
                    <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-3xl p-4 flex items-center gap-3 border border-orange-100 shadow-sm relative overflow-hidden group hover:scale-[1.02] transition-transform cursor-pointer" onClick={() => setShowSnaps(true)}>
                        <div className="absolute -right-4 -top-4 opacity-10 rotate-12 transition-transform group-hover:rotate-45 duration-700">
                            <Flame size={80} />
                        </div>
                        <div className="w-10 h-10 bg-white rounded-2xl text-orange-500 shadow-sm flex items-center justify-center relative z-10">
                            <Flame size={20} fill="currentColor" className="animate-pulse" />
                        </div>
                        <div className="relative z-10">
                            <p className="text-[9px] font-bold text-orange-400 uppercase tracking-wider">Modo Racha</p>
                            <p className="font-heading font-black text-orange-600 text-lg">{streak} Días</p>
                        </div>
                    </div>
                 </div>
            </div>
        ...
        <main className="flex-1 h-[100dvh] overflow-y-auto custom-scrollbar relative scroll-smooth bg-surface">
             <div className="p-4 lg:p-8 max-w-[1400px] mx-auto pb-48 lg:pb-12">
                 ...
                 <div className="hidden lg:flex justify-between items-center mb-8 sticky top-0 z-30 py-4 bg-surface/90 backdrop-blur-md">
                    <div>
                        <h1 className="text-2xl font-heading font-black text-slate-900">{getPageTitle()}</h1>
                        <p className="text-xs text-slate-500 font-medium">Bienvenido de nuevo, {displayName.split(' ')[0]}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative group">
                            ...
                            <input type="text" placeholder="Buscar..." className="bg-white pl-10 pr-4 py-2.5 rounded-2xl shadow-sm border border-slate-100 focus:ring-4 focus:ring-brand-500/10 outline-none w-56 transition-all focus:w-72 font-medium text-xs text-slate-700" />
                        </div>

                        <div className="w-10 h-10 bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center justify-center text-slate-400 hover:text-brand-500 hover:shadow-md transition-all cursor-pointer relative">
                             <Bell size={18} />
                             {notifications.length > 0 && <span className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-rose-500 rounded-full"></span>}
                        </div>

                        <div className="flex items-center gap-3 bg-white p-1.5 pr-4 rounded-[1rem] border border-slate-100 shadow-sm cursor-pointer hover:shadow-md transition-all" onClick={() => setActiveView('settings')}>
                             <div className="w-8 h-8 rounded-xl overflow-hidden bg-slate-50">
                                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${avatarSeed}`} className="w-full h-full object-cover" />
                             </div>
                             <div className="text-left">
                                 <p className="font-bold text-[11px] text-slate-900">{displayName}</p>
                                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{session.user.email}</p>
                             </div>
                        </div>
                    </div>
                 </div>
                 ...
                 <div className="lg:hidden flex justify-between items-center mb-6 sticky top-0 bg-surface/80 backdrop-blur-xl z-30 py-4 px-2 -mx-2 border-b border-slate-100/50">
                     <div className="flex items-center gap-3">
                         <div className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center border border-slate-100">
                             <Mascot variant="idle" size={32} />
                         </div>
                         <div>
                             <p className="font-heading font-bold text-slate-400 text-[10px] uppercase mb-0 tracking-wider">Buenos días,</p>
                             <h1 className="font-heading font-black text-lg text-slate-900 leading-none truncate max-w-[150px]">{displayName.split(' ')[0]}</h1>
                         </div>
                     </div>
                     <div className="w-10 h-10 bg-white rounded-2xl border border-slate-100 p-0.5 shadow-sm overflow-hidden active:scale-95 transition-transform" onClick={() => setActiveView('settings')}>
                 ...
                 <div className="space-y-6 animate-slide-up">
                    {activeView === 'dashboard' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-2xl flex items-center gap-3 text-xs text-indigo-800 font-medium">
                                <Mascot variant="thinking" size={24} />
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                                ...
                                <div className="xl:col-span-8 space-y-6">
                                    <div className="md:h-[380px] h-[300px]"><FinancialChart transactions={transactions} /></div>
                                ...
                                <div className="xl:col-span-4 space-y-6 sticky top-24">
                                    ...
                                    <AIAdvisor transactions={transactions} streak={streak} className="h-[calc(100vh-400px)] min-h-[400px]" />
                                </div>
                            </div>
                        </div>
                    )}
